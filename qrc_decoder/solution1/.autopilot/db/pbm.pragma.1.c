# 1 "qrc_decoder/src/pbm.c"
# 1 "qrc_decoder/src/pbm.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 147 "<built-in>" 3
# 1 "<command line>" 1
...
# 8 "<command line>" 2
# 1 "<built-in>" 2
# 1 "qrc_decoder/src/pbm.c" 2
# 19 "qrc_decoder/src/pbm.c"
...
static byte nextint(FILE* f)
{
 ...
 do
 {
  if (feof(f))
  {
   fprintf((&__iob_func()[2]), "Unexpected end of file\n");
   exit(1);
  }
  c = fgetc(f);
  ...
  do
  {
   if (feof(f))
   {
    fprintf((&__iob_func()[2]), "Unexpected end of file in comment\n");
    exit(1);
   }
   c = fgetc(f);
  }
 ...
 if (!('0' <= c && c <= '9'))
 {
  fprintf((&__iob_func()[2]), "Unexpected character '%c'\n", c);
  exit(1);
 }
...
void data_write (unsigned char dataout)
{
    ...
    for (y=0;y<12;y++) {
          ...
          fprintf(fp, "%c", dataout);
       }
    fclose(fp);
 }
void load_pbm2(unsigned char pdata[49*49], FILE* f)
{_ssdm_SpecArrayDimSize(pdata, 2401);
 fgetc(f);
 char format = fgetc(f);
 byte w = nextint(f);
 byte h = nextint(f);
 ...
 else if (format == '4')
 {
  ...
  {
   for (size_t j = 0; j < w; j++)
    pdata[i*h+j] = nextbit(f, &buf, &buf_avail);
...
void load_pbm(scanner_t* scanner, FILE* f)
{
 ...
 if (w != h)
 {
  fprintf((&__iob_func()[2]), "I only handle square images\n");
  exit(1);
 }
 ...
 if (!d)
 {
  fprintf((&__iob_func()[2]), "Could not allocate memory\n");
  exit(1);
 }
 ...
 if (format == '1')
 {
  for (size_t i = 0; i < h; i++)
  for (size_t j = 0; j < w; j++)
   (scanner->d[(i)*scanner->s + (j)]) = nextint(f);
 }
 else if (format == '4')
 {
  byte buf = 0;
  ...
  {
   for (size_t j = 0; j < w; j++)
    (scanner->d[(i)*scanner->s + (j)]) = nextbit(f, &buf, &buf_avail);
   buf_avail = 0;
  }
 }
 else
 {
  fprintf((&__iob_func()[2]), "Unsupported format '%c'\n", format);
  exit(1);
 }
}
